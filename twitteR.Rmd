...
author: "Anna Qualieri & Saskia Freytag"
output:
  pdf_document:
    fig_caption: yes
    fig_height: 6
...
# Chunk options
knitr::opts_chunk$set(fig.width=8, fig.height=6, echo=T, warning=FALSE, message=FALSE,prompt=T,tidy=T,tidy.opts=list(width.cutoff=40),include=TRUE,cache=FALSE)

require("png")
require("grid")
dir <- "/Users/quaglieri.a/Documents/varie/Rladies/twitter_workshop"
...
Before you can download data from Twitter's data you need to setup your API by becoming a Twitter developer. First, to become a twitter developer you need to have a twitter account. Then, you can sign up [here](https://dev.twitter.com/resources/signup) (Figure \ref{fig:fig1}). Once you have signed up click on the *My Apps* buttom on the left panel as in Figure \ref{fig:fig2} and then click on *Create a new app*. You should provide a brief description about your App and a few other details. For today's workshop we set up the the *RLadies* App shown in Figure \ref{fig:fig3}. By clicking onto your App's name you can now manage and see your settings as well as the codes that you need to get started with R. You can find the keys that you need in the *Keys and Access Tokens* button. Here you can find your four codes (highlighted in Figure \ref{fig:fig4}) to use into R. 

```{r echo = FALSE, fig.cap="\\label{fig:fig1}Signup at https://dev.twitter.com/resources/signup.", fig.width=5, fig.height=5}
img <- readPNG(file.path(dir,"Figures","signup1.png"))
grid.raster(img)
```

```{r echo = FALSE, fig.cap="\\label{fig:fig2}After signing up click on 'My Apps' and then 'Create a new App's.", fig.width=5, fig.height=5}
img <- readPNG(file.path(dir,"Figures","signup2.png"))
grid.raster(img)
```

```{r echo = FALSE, fig.cap="\\label{fig:fig3}You can now manage your App's settings.", fig.width=5, fig.height=5}
img <- readPNG(file.path(dir,"Figures","twitter_apps.png"))
grid.raster(img)
```

```{r echo = FALSE, fig.cap="\\label{fig:fig4}This is where you can find the access keys that you need to get started with R.", fig.width=5, fig.height=5}
...
RuPaul's drag race is an American reality competitions showing RuPaul's search for "America's next drag superstar". Just to tease your curiosity, Figure \ref{fig:fig5} shows you all the Queens which are competing in Season 9!

```{r echo = FALSE, fig.cap="\\label{fig:fig5}Drag queens competing in RuPaul's drag race Season 9.", fig.width=6, fig.height=6}
...
Another functionlaity of *tweetR* is to download the information about the users that wrote the the tweets. This can be done with the functions *getUser()* and for multiple lookups with *lookupUsers()* and . Of course, you are able to access user's information according to the user's permission. Therefore, you normally expect a lot of missing or fake data! The search can be done by providing on of *screenNames* or *userIDs*.
...
### Which other question can be asked?
